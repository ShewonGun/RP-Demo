import { FiX, FiCheckCircle, FiAlertTriangle, FiAlertOctagon, FiMapPin } from "react-icons/fi";
...
const ZoneDetailModal = ({ open, zone, onClose }) => {
    if (!open || !zone) return null;

    const s = riskStyles[zone.risk] || riskStyles.Low;
    const { Icon } = s;
    ...
    return (
        ...
        >
            <div
                className="max-h-[88vh] w-full max-w-md overflow-y-auto rounded-sm border border-slate-200 bg-white p-5 shadow-xl dark:border-slate-800 dark:bg-slate-900"
            ...
            >
                ...
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-semibold text-slate-900 dark:text-white">{zone.name}</h2>
                            <p className="text-xs text-slate-400 dark:text-slate-500">Distribution Zone</p>
                        </div>
                    </div>
                ...
                <div className="mt-4">
                    <div className="flex items-center justify-between mb-1.5">
                        <span className="text-xs font-medium text-slate-700 dark:text-slate-300">Estimated Storage Level</span>
                        <span className="text-sm font-semibold text-slate-900 dark:text-white">{zone.storagePercentage}%</span>
                    </div>
                    <div className="h-3 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                        <div className={`h-full rounded-full ${s.bar}`} style={{ width: `${zone.storagePercentage}%` }} />
                    </div>
                </div>

                {/* Detail rows */}
                <div className="mt-4 rounded-sm border border-slate-200 dark:border-slate-800 px-4">
                    <Row label="Zone ID" value={zone.id} />
                    <Row label="Households" value={`${zone.households} connections`} />
                    <Row label="Average Storage" value={`${zone.storagePercentage}%`} />
                    <Row label="Predicted Daily Demand" value={`${zone.predictedDemand.toLocaleString()} L`} />
                    <Row label="Risk Level" value={zone.risk} />
                    <Row label="Priority Rank" value={`#${zone.priority} of 3`} />
                    <Row label="Data Source" value="Simulated" />
                </div>

                {/* Priority reasoning */}
                <div className="mt-4">
                    <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">Priority Reasoning</p>
                    <ul className="space-y-1.5">
                        {zone.priorityReason.map((r, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs text-slate-500 dark:text-slate-400">
                                <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400 dark:bg-slate-600" />
                                {r}
                            </li>
                        ))}
                    </ul>
